...
import { Search, Plus, Filter, Link, Unlink, ChevronRight, ChevronDown, Edit2, Trash2, Activity, MapPin, X, Calendar, ClipboardList, Beef, Loader2, CheckCircle, XCircle, Baby, Pencil, Save, Tractor, PawPrint, SlidersHorizontal, ChevronLeft, Camera, ImagePlus, LineChart, Sparkles, Edit3, Dna } from 'lucide-react';
...
export default function ManajemenTernak() {
  ...
  const [filterBtnRef, setFilterBtnRef] = useState(null);
  ...
  return (
    <>
      {isSelectMode && (
        <>
          {/* Mobile Floating Bottom Bar for Select Mode (Visible only on md and below) */}
          <div className="fixed md:hidden z-50 flex items-center justify-between px-4 gap-2" style={{
            ...
            <button onClick={() => { setIsSelectMode(false); setSelectedForDelete([]); }} className="text-gray-500 font-bold text-[13px] sm:text-[14px] shrink-0">
              {t.livestock_select_mode_cancel || 'Batal'}
            </button>
            <div className="text-[13px] sm:text-[14px] font-bold text-gray-900 flex-1 text-center truncate px-1">
              {selectedForDelete.length > 0
                ? (t.livestock_select_mode_selected || '{count} Dipilih').replace('{count}', selectedForDelete.length)
                : (t.livestock_select_mode_title || (lang === 'id' ? 'Pilih' : 'Select'))}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => { if (selectedForDelete.length === filteredSapi.length) { setSelectedForDelete([]); } else { setSelectedForDelete(filteredSapi.map(s => s.id)); } }} className="text-[#2E7D32] font-bold text-[13px] sm:text-[14px]">
              ...
              <button onClick={handleBulkDelete} disabled={selectedForDelete.length === 0} className={`flex items-center justify-center p-2 rounded-full transition-colors ${selectedForDelete.length > 0 ? 'bg-red-500 text-white shadow-md' : 'bg-gray-100 text-gray-400'}`}>
                <Trash2 size={18} />
              </button>
            </div>
          </div>

...
export default function DetailTernak() {
  ...
  const [activeDetailTab, setActiveDetailTab] = useState('catatan');
  ...
  const handleConfirmDeleteIB = async () => {
    if (!deleteIBTarget) return;
    
    try {
      const res = await axiosInstance.delete(`/scanner/reproduksi/record/${deleteIBTarget.id}`);
      if (res.data?.success) {
        const updatedSapi = {
          ...selectedSapi,
        ...
        toast.error('Gagal menghapus catatan IB');
      }
    } catch (error) {
      console.error(error);
      toast.error('Gagal menghapus catatan IB');
      ...
      {/* ── MOBILE VIEW ── */}
        ...
        <div className="lg:hidden fixed inset-0 z-[35] bg-[#F3F4F6] overflow-y-auto animate-in slide-in-from-bottom duration-300 no-scrollbar pb-[100px]">
          ...
          <div className="relative z-10 bg-[#F3F4F6] min-h-[calc(100vh-200px)] -mt-6 rounded-t-[32px] shadow-[0_-12px_30px_rgba(0,0,0,0.1)] overflow-hidden">
            ...
            <div className="px-4 py-5 grid grid-cols-3 gap-2 bg-white">
              {/* Tab 1: Catatan Ternak */}
            <button 
              onClick={() => setActiveDetailTab('catatan')}
            ...
            >
               ...
               <span className="font-bold text-[10px] tracking-wide text-center leading-tight">{lang === 'id' ? 'Catatan Ternak' : 'Records'}</span>
            </button>
        ...
        {/* Birth Confirmation Modal */}{/* Birth Confirmation Modal */}

  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const dailyStats = {};
        filteredTelemetry.forEach(d => {
          const dt = new Date(d.batch_ts || d.created_at);
      ...
      >
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            {[
              { key: '1hr',  label: lang === 'id' ? '1 Jam' : '1 Hr' },
              { key: '1wk',  label: lang === 'id' ? '1 Mg' : '1 Wk' },
              { key: '1bln', label: lang === 'id' ? '1 Bln' : '1 Mo' },
              { key: '3bln', label: lang === 'id' ? '3 Bln' : '3 Mo' },
              { key: '6bln', label: lang === 'id' ? '6 Bln' : '6 Mo' },
              { key: '1th',  label: lang === 'id' ? '1 Th' : '1 Yr' },
            ].map(opt => (
              <button
                key={opt.key}
      ...
      <div className="transition-opacity duration-300 opacity-100 flex flex-col gap-6">
              ...
              <div style={{ background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: '16px', boxShadow: 'var(--shadow-card)', padding: '24px' }}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                  <h3 className="text-lg font-semibold text-[var(--text-1)] font-display">{t.sensor_chart_telemetry || (lang === 'id' ? "Suhu & Aktivitas" : "Temperature & Activity")}</h3>
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center gap-4 text-xs font-medium">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[var(--text-2)]">{t.sensor_chart_temp || (lang === 'id' ? "Suhu" : "Temperature")}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full bg-[var(--color-forest)]"></span>
                        <span className="text-[var(--text-2)]">{t.sensor_chart_activity || (lang === 'id' ? "Aktivitas" : "Activity")}</span>
                      </div>
                    </div>
                  </div>
                </div>

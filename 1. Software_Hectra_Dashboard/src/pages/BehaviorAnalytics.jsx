  ...
  useEffect(() => {
    const fetchCows = async () => {
      try {
        ...
        toast.error('Gagal memuat daftar sapi.');
      }
    };
    fetchCows();
  }, []);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-[var(--color-text-primary)]">Behavior Analytics</h1>
          <p className="text-[var(--color-text-secondary)] mt-1">Analisis mendalam pola perilaku ternak berdasarkan data historis sensor.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border)', borderRadius: '16px', boxShadow: 'var(--shadow-card)', padding: '24px' }} className="lg:col-span-4">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)] font-display mb-4">Distribusi Aktivitas (Hari Ini)</h2>
          ...
          <div className="h-[250px]">
            {pieData.length === 0 || pieData.every(item => item.value === 0) ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-4">
                ...
                <p className="text-sm font-medium text-[var(--color-text-secondary)]">Tidak ada data sensor hari ini</p>
              </div>
            ) : (
        ...
        <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border)', borderRadius: '16px', boxShadow: 'var(--shadow-card)', padding: '24px' }} className="lg:col-span-8 flex flex-col">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)] font-display mb-6">Komparasi Perilaku Mingguan</h2>

          <div className="flex-1 min-h-[300px]">
            {weeklyData.length === 0 || weeklyData.every(item => item.aktif === 0 && item.makan === 0 && item.istirahat === 0) ? (
              <div className="h-full min-h-[300px] flex flex-col items-center justify-center text-center p-4">
                <Activity className="w-8 h-8 text-[var(--color-text-muted)] mb-2" />
                <p className="text-sm font-medium text-[var(--color-text-secondary)]">Belum ada data sensor mingguan</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="aktif" name="Aktif / Estrus" stackId="a" fill="var(--color-gold)" radius={[0, 0, 4, 4]} />
                  <Bar dataKey="makan" name="Makan / Memamah Biak" stackId="a" fill="var(--color-forest)" />
                  <Bar dataKey="istirahat" name="Istirahat / Tidur" stackId="a" fill="var(--color-sage)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>

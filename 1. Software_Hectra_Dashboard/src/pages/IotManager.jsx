...
export default function IotManager() {
  ...
  const handlePairSubmit = async (e) => {
    ...
    if (!selectedDevice || !selectedCow) {
      toast.error("Harap pilih kalung dan sapi terlebih dahulu!");
      return;
    }
    ...
    try {
      ...
      toast.error("Gagal memasangkan kalung: " + (err.response?.data?.detail || err.message));
    } finally {
      setPairing(false);
    }
  };
  ...
  const handleConfirmUnpair = async () => {
    ...
    try {
      ...
      fetchData();
      toast.success(`Kalung ${pendingUnpair.device_id} berhasil dilepaskan.`);
    } catch (err) {
      toast.error("Gagal melepas kalung: " + (err.response?.data?.detail || err.message));
    } finally {
      setUnpairing(false);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-[var(--color-text-primary)]">Manajemen Perangkat IoT</h1>
          <p className="text-[var(--color-text-secondary)] mt-1">Pasangkan kalung sensor cerdas ke profil sapi untuk memantau data kesehatan secara real-time.</p>
        </div>
        <button 
          onClick={fetchData} 
      ...
      <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border)', borderRadius: '24px', boxShadow: 'var(--shadow-card)', padding: '24px' }}>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-bold text-[var(--color-text-primary)] font-display">Pasangkan Kalung Ke Sapi</h2>
        </div>
        ...
        <form onSubmit={handlePairSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Devices dropdown */}
            <div>
              <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Pilih Perangkat Kalung</label>
              <select
              ...
              >
                <option value="" style={{ background: 'var(--bg-card)', color: 'var(--text-3)' }}>— Pilih Kalung —</option>
                {unassignedDevices.map(d => (
                  <option key={d.device_id} value={d.device_id} style={{ background: 'var(--bg-card)', color: 'var(--text-1)' }}>{d.device_id}</option>
                ))}
                {unassignedDevices.length === 0 && (
                  <option key="no-collars" disabled style={{ background: 'var(--bg-card)', color: 'var(--text-3)' }}>Semua kalung sudah terpasang</option>
                )}
              </select>
            </div>
            ...
            {/* Cattle dropdown */}
            <div>
              <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Pilih Sapi</label>
              <select
                value={selectedCow}
          ...
          {isWorker ? (
            <div className="flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 font-bold text-sm">
              ...
              <span>HAK AKSES DITOLAK (PEKERJA HANYA DAPAT MEMBACA DATA)</span>
            </div>
          ) : (
            <button
              type="submit"
      ...
      <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border)', borderRadius: '24px', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
        <div className="px-6 py-5 border-b border-[var(--border)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold text-[var(--color-text-primary)] font-display">Daftar Perangkat Aktif</h2>
        ...
        {loading ? (
          <div className="py-20 text-center flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] italic">Memuat data perangkat...</p>
          </div>
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-semibold">Belum ada perangkat dipasangkan</p>
            <p className="text-xs text-slate-400 mt-1">Gunakan formulir di atas untuk mulai memasangkan kalung ke sapi.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-[var(--border)]">
              <thead style={{ background: 'var(--bg-hover)' }}>
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">ID Kalung</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Dipasang Pada Sapi</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Status Baterai</th>
                  <th scope="col" className="px-6 py-3 text-right text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Aksi</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={d.device_id} className="hover:bg-[var(--bg-hover)] transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {isWorker ? (
                          <span className="text-xs text-[var(--color-text-muted)] italic">Terkunci (Pekerja)</span>
                        ) : (
                          <button
      ...
      {showUnpairModal && pendingUnpair && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          <div style={{ background: 'var(--bg-surface)', border: '0.5px solid var(--border)', borderRadius: '24px', boxShadow: 'var(--shadow-modal)' }} className="p-6 max-w-sm w-full relative z-10 text-center animate-in scale-in duration-200">
            ...
            <h3 className="text-lg font-bold text-[var(--color-text-primary)] font-display mb-2">Lepaskan Kalung?</h3>
